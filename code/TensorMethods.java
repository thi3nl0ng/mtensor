 ...
 * Modified by HuyDQ to process with datasets that value column is not the last one, it can be any order
 */
public class TensorMethods {
    ...
    public static Tensor importTensor(final String path, final int dimension) throws IOException {
        return importTensor(path, " ", dimension, dimension);
    }
     ...
     * @throws IOException
     */
    public static Tensor importTensor(final String path, final String delim, final int dimension, final int valOrd) throws IOException{
        ...
        while(true){
            ...
            if(tokens.length < dimension) {
                System.out.println("Skipped Line Type 1: " + line);
                continue;
            }
            for(int dim = 0, inc = 0; dim < dimension; dim++) {
            	
            	inc = (dim < valOrd?dim:(dim+1));
            	
                if(!strToIntValue[dim].containsKey(tokens[inc])) {
                    strToIntValue[dim].put(tokens[inc], strToIntValue[dim].size());
                }

                int index = strToIntValue[dim].get(tokens[inc]);
        ...
        Tensor tensor = importTensor(path, delim, dimension, valOrd, omega, cardinalities, attributeToValueToNumArr, strToIntValue);
     ...
     * @throws IOException
     */
    private static Tensor importTensor(final String path, final String delim, final int dimension, final int valOrd, final int omega, final int[] cardinalities, int[][] attributeToValueToNum, Map<String, Integer>[] strToIntValue) throws IOException {
        ...
        while (true) {
           ...
           if(tokens.length < dimension) {
                System.out.println("Skipped Line Type 2: " + line);
                continue;
            }
            for (int dim = 0, inc = 0; dim < dimension; dim++) {
            	inc = (dim < valOrd?dim:(dim+1));
            ...
            if (valOrd >= tokens.length)
            	values[curTupleNum] = 1;
            else
            	values[curTupleNum] = Double.valueOf(tokens[valOrd]).intValue();
